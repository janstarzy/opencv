...
/// cast jlong to Mat*, throw for null
static Mat* getMat(jlong self) {
  if(!self) throw std::invalid_argument("null matrix");
  return (Mat*)self;
}
  ...
  (JNIEnv* env, jclass, jlong self, jlong m_nativeObj, jlong mask_nativeObj)
{
    ...
    try {
        ...
        Mat* me = getMat(self);
        Mat* m = getMat(m_nativeObj);
        Mat* mask = getMat(mask_nativeObj);
        me->copyTo( *m, *mask );
...
JNIEXPORT jlong JNICALL Java_org_opencv_core_Mat_n_1dataAddr
  (JNIEnv* env, jclass, jlong self)
{
    static const char method_name[] = "Mat::n_1dataAddr()";
    try {
        LOGD(method_name);
        Mat* me = getMat(self);
  ...
  (JNIEnv* env, jclass, jlong self, jlong value_nativeObj, jlong mask_nativeObj)
{
    ...
    try {
        LOGD(method_name);
        Mat* me = getMat(self);
        Mat* value = getMat(value_nativeObj);
        Mat* mask = getMat(mask_nativeObj);
    ...
    (JNIEnv* env, jclass, jlong self, jint row, jint col, jint count, jdoubleArray vals)
{
    ...
    try {
        LOGD(method_name);
        cv::Mat* me = getMat(self);
        if(!me->data) return 0;  // empty data
